...
npm install -g agent-ai-cli

# Dev dependency (for programmatic use)
npm install --save-dev agent-ai-cli
```
...
```typescript
import { createGuardrail } from 'agent-ai-cli';
...
Create `.agent-ai-cli.json` in your project root:
...
Or `~/.agent-ai-cli/config.json` for global settings.
...
```typescript
import { AgentCLI, createGuardrail } from 'agent-ai-cli';
...
```typescript
import { AgentCLI, LiveDebugger } from 'agent-ai-cli';
...
```typescript
import { AgentCLI, createWebServer } from 'agent-ai-cli';
