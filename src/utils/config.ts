...
const DEFAULT_CONFIG: AIAgentConfig = {
  ...
  model: undefined,
  port: 3000,
  sessionDir: path.join(os.homedir(), '.vibe-agent', 'sessions'),
  guardrails: [],
  logLevel: 'info',
};
...
function getConfigPaths(): string[] {
  return [
    path.join(process.cwd(), '.vibe-agent.json'),
    path.join(process.cwd(), '.vibe-agent.yaml'),
    path.join(os.homedir(), '.vibe-agent', 'config.json'),
    path.join(os.homedir(), '.vibe-agent', 'config.yaml'),
  ];
}
...
export function saveConfig(config: Partial<AIAgentConfig>): void {
  const configDir = path.join(os.homedir(), '.vibe-agent');
