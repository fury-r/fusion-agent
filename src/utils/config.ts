...
const DEFAULT_CONFIG: AIAgentConfig = {
  ...
  model: undefined,
  port: 3000,
  sessionDir: path.join(os.homedir(), '.fusion-agent', 'sessions'),
  guardrails: [],
  logLevel: 'info',
};
...
function getConfigPaths(): string[] {
  return [
    path.join(process.cwd(), '.fusion-agent.json'),
    path.join(process.cwd(), '.fusion-agent.yaml'),
    path.join(os.homedir(), '.fusion-agent', 'config.json'),
    path.join(os.homedir(), '.fusion-agent', 'config.yaml'),
  ];
}
...
export function saveConfig(config: Partial<AIAgentConfig>): void {
  const configDir = path.join(os.homedir(), '.fusion-agent');
